# Hi, I'm Ankit Aggarwal 👋

**CS Student @ University of British Columbia · Vancouver, Canada**

[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ankit.acoolguy@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/jhonny1677)

---

## About Me

- 🎓 Computer Science student at the **University of British Columbia**, Vancouver
- 🤖 Passionate about **Machine Learning** and **Data Science** — building predictive models and extracting insights from real-world data
- 📱 Also love building full-stack and mobile apps with **React Native**, the **MERN stack**, and **TypeScript**
- 🌱 Currently exploring **deep learning** and **NLP**
- 📫 Reach me at **ankit.acoolguy@gmail.com**

---

## 🛠️ Tech Stack

**ML & Data Science**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

**Web & Mobile**

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=flat&logo=express&logoColor=white)

**Other**

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## 🚀 Featured Projects

| Project | Description | Tech |
|---|---|---|
| [Hindi Learning App](https://github.com/jhonny1677/hindi-learning-mobile-app) | AI-powered mobile language learning platform with gamification & offline support | React Native · TypeScript · Expo |
| [Real Estate Platform](https://github.com/jhonny1677/Real-Estate-App-MERN) | Full-stack platform for listing, buying, and searching properties | MERN Stack |
| [Canada Services API](https://github.com/jhonny1677/Canada-Services-API) | REST API for essential services across top Canadian cities | Node.js |
| [Pneumonia Detector](https://github.com/jhonny1677/Pneumonia_Detector) | Deep learning model to detect pneumonia from chest X-rays | Python · TensorFlow |
| [Diabetes Predictor](https://github.com/jhonny1677/Diabetes_Predictor) | ML classifier to predict diabetes risk from clinical data | Python · Scikit-Learn |
| [Air Pollution Forecast](https://github.com/jhonny1677/Air_Pollution_Forecast) | Time-series forecasting of air quality metrics | Python · Pandas |

---

## 📊 GitHub Stats

![Ankit's GitHub Stats](https://github-readme-stats.vercel.app/api?username=jhonny1677&show_icons=true&theme=tokyonight&hide_border=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=jhonny1677&layout=compact&theme=tokyonight&hide_border=true)

---

⭐ If you find my work interesting, feel free to star a repo or connect!
